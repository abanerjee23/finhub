...
Self-contained demo loop in the UI (`frontend/src/App.tsx`):

- **Workbench Controls** — reset/seed queue, run agent processing (sweep), refresh
- **Analytics** — KPIs, operator status breakdown, owner chart, stage times
- **Ticket detail** — agent diagnosis hero, metadata, comments, proof attachments, activity log
- **Search Tickets** — filter, inline `operator_status` dropdown

API: `src/cfin_agents/api.py`. Workbench endpoints under `/api/workbench/*`. Ticket transitions use `operator_status` (not legacy `status` + policy flags).
